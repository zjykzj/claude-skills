...
- **Per-plugin versioning**: this repo has no repo-level version — each plugin versions and releases independently (plugin-scoped CHANGELOG headers, `<plugin>--vX.Y.Z` tags, per-plugin GitHub Releases); the release flow lives in `docs/release-versioning.md`
